"""
AI 客户端层 — 统一通过 OpenAI 兼容接口调用（参考 Nehchat 重写）。

支持流式对话与连通性测试，带指数退避重试与思考链剥离；
供应商 / 模型由数据库目录解析后传入。
"""
...
class AIClient:
    ...
    def _get_client(self, base_url: str, api_key: str, timeout: float = 120) -> AsyncOpenAI:
    ...
    def _reject_thinking_extra(self, base_url: str) -> None:
        if base_url not in self._thinking_extra_rejected:
            ...
            logger.info("供应商不接受关闭思考参数，后续不再附带: %s", base_url)
    ...
    async def stream_chat(
        ...
        for attempt in range(_API_RETRY_MAX + 1):
            try:
                ...
                if started:
                    logger.warning("流式响应已开始后中断（不重试）: %s", e)
                ...
                if isinstance(e, RateLimitError) or status == 429:
                    logger.warning("API 速率限制 (尝试 %d): %s", attempt + 1, e)
                ...
                if isinstance(e, (APITimeoutError, httpx.TimeoutException)):
                    logger.warning("API 请求超时 (尝试 %d): %s", attempt + 1, e)
                    if attempt < _API_RETRY_MAX:
                ...
                if isinstance(e, (APIConnectionError, httpx.NetworkError)):
                    logger.warning("网络错误 (尝试 %d): %s", attempt + 1, e)
                    if attempt < _API_RETRY_MAX:
                        await asyncio.sleep(_API_RETRY_DELAY * (2 ** attempt))
                        continue
                ...
                if isinstance(e, APIStatusError):
                    ...
                    if status is not None and status >= 500:
                        logger.error("API 服务错误 (尝试 %d): %s", attempt + 1, e)
                        if attempt < _API_RETRY_MAX:
                            await asyncio.sleep(_API_RETRY_DELAY * (2 ** attempt))
                            continue
                ...
                logger.warning("API 请求失败 (尝试 %d): %s", attempt + 1, e)
                if attempt < _API_RETRY_MAX:
                    await asyncio.sleep(_API_RETRY_DELAY * (2 ** attempt))
                    continue
    ...
    async def test_hello(self, model_id: str, base_url: str, api_key: str) -> dict:
        ...
        try:
            ...
            return {"ok": True, "latency_ms": latency, "preview": text[:200]}
            ...
            logger.warning("模型测试失败: %s", e, exc_info=True)
